...
export default class extends Vue {
  ...
  // 解锁浏览器音频自动播放限制（用户首次点击/按键后触发）
  unlockAudio() {
    const that = this as any
    const unlock = () => {
      const tryUnlock = (audio: any) => {
        if (!audio) return
        audio.volume = 0
        const p = audio.play()
        if (p && p.then) {
      ...
      tryUnlock(that.$refs.audioVo)
      tryUnlock(that.$refs.audioVo2)
    ...
    if (typeof WebSocket == 'undefined') {
      ...
      this.websocket.onmessage = function (msg) {
        try {
          ...
          // 心跳消息(type=0)仅用于保持连接，不弹窗、不播放声音
          if (jsonMsg.type === 0) return

          // 重置音频播放位置
          if (that.$refs.audioVo) that.$refs.audioVo.currentTime = 0
          if (that.$refs.audioVo2) that.$refs.audioVo2.currentTime = 0

          // 播放提示音（type=1 新订单，type=2 催单）
          ...
          if (jsonMsg.type === 1) {
            playAudio(that.$refs.audioVo)
          } else if (jsonMsg.type === 2) {
            playAudio(that.$refs.audioVo2)
          }

          // 配送/自提标签
          const deliveryTag = jsonMsg.deliveryType === 2
            ? '<span style="display:inline-block;padding:2px 8px;background:#FF9800;color:#fff;border-radius:4px;font-size:12px;margin-right:6px;">自提</span>'
            : jsonMsg.deliveryType === 1
            ? '<span style="display:inline-block;padding:2px 8px;background:#4CAF50;color:#fff;border-radius:4px;font-size:12px;margin-right:6px;">配送</span>'
            : ''
          ...
          that.$notify({
            ...
            message: `${
              jsonMsg.type === 1
                ? `<span>${deliveryTag}您有1个<span style=color:#419EFF>订单待处理</span>,${jsonMsg.content},请及时接单</span>`
                : `${deliveryTag}${jsonMsg.content}<span style='color:#419EFF;cursor: pointer'>去处理</span>`
            }`,
          })
        } catch (e) {
