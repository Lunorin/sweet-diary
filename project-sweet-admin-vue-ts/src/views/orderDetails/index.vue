  ...
  <div class="dashboard-container">
    ...
    <div class="container" :class="{ hContainer: tableData.length }">
      ...
      >
        ...
        <!-- 配送类型 -->
        <el-table-column key="deliveryType" label="配送类型" align="center" width="90">
          <template slot-scope="{ row }">
            <el-tag v-if="row.deliveryType === 2" type="warning" size="small">自提</el-tag>
            <el-tag v-else type="success" size="small">配送</el-tag>
          </template>
        </el-table-column>
        <!-- 取单号 -->
        <el-table-column key="pickupNo" prop="pickupNo" label="取单号" align="center" width="100">
          <template slot-scope="{ row }">
            <span v-if="row.deliveryType === 2 && row.pickupNo" style="color:#f56c6c;font-weight:bold">{{ row.pickupNo }}</span>
            <span v-else>-</span>
          </template>
        </el-table-column>
        <el-table-column
        ...
        <el-table-column
          v-if="[0, 6, 7].includes(orderStatus)"
          key="consignee"
          prop="consignee"
          label="用户名"
        ...
        />
        <el-table-column
          v-if="[0, 2, 3, 4, 5, 6, 7].includes(orderStatus)"
          key="address"
          prop="address"
          label="地址"
          :class-name="orderStatus === 7 ? 'address' : ''"
        />
        <el-table-column
          v-if="[0, 7].includes(orderStatus)"
          key="orderTime"
          prop="orderTime"
          label="下单时间"
        ...
        <el-table-column
          ...
          :min-width="[7].includes(orderStatus) ? 80 : 'auto'"
        />
        <el-table-column
        ...
        >
          <template slot-scope="{ row }">
            <span v-if="row.deliveryType === 2">{{ row.pickupAppointmentTime || '即时取餐' }}</span>
            <span v-else>{{ row.estimatedDeliveryTime }}</span>
          </template>
        </el-table-column>
        <el-table-column
          v-if="[0, 2, 6].includes(orderStatus)"
          key="amount"
          prop="amount"
          label="实收金额"
          align="center"
        ...
        <el-table-column
          v-if="[2, 3, 4, 5, 6].includes(orderStatus)"
          key="remark"
          prop="remark"
          label="备注"
          align="center"
          ...
          <template slot-scope="{ row }">
            ...
            <div class="before">
              ...
              <el-button
                ...
                type="text"
                class="blueBug"
                @click="handleCompletePreparation(row)"
              >
                制作完成
              </el-button>
              <!-- 待派送：派送 -->
    ...
    >
        ...
        <div class="order-top">
          <div>
            ...
            <div
              ...
              class="order-status"
              :class="{ status3: [3, 4, 5].includes(dialogOrderStatus) }"
            >
              {{
        ...
        <div class="order-middle">
          <div class="user-info">
            <div class="user-info-box">
              ...
              <div
                v-if="[2, 3, 4, 5, 6].includes(dialogOrderStatus)"
                class="user-getTime"
              >
                <label>{{
                  dialogOrderStatus === 6
                    ? (diaForm.deliveryType === 2 ? '取餐时间：' : '送达时间：')
                    : (diaForm.deliveryType === 2 ? '预约取餐时间：' : '预计送达时间：')
                }}</label>
                <span>{{
                  dialogOrderStatus === 6
                    ? diaForm.deliveryTime
                    : (diaForm.deliveryType === 2
                        ? (diaForm.pickupAppointmentTime || '即时取餐')
            ...
            <div
              class="user-remark"
              :class="{ orderCancel: dialogOrderStatus === 7 }"
            >
              <div>{{ dialogOrderStatus === 7 ? '取消原因' : '备注' }}</div>
              <span>{{
                dialogOrderStatus === 7
          ...
          <div class="amount-info">
            ...
            <div class="amount-list">
              <div class="dish-amount">
                <span class="amount-name">甜品小计：</span>
      ...
      <span v-if="dialogOrderStatus !== 7" slot="footer" class="dialog-footer">
        <el-checkbox
        ...
        <el-button
          v-if="[1, 3, 4, 5, 6].includes(dialogOrderStatus)"
...
export default class extends Vue {
  ...
  cancelOrDeliveryOrComplete(status: number, id: string) {
    ...
    ;(status === 4 ? deliveryOrder : completeOrder)(params)
      .then((res) => {
        if (res.data.code === 1) {
          this.$message.success('操作成功')
          this.orderId = ''
  ...
  // 制作完成
  handleCompletePreparation(row: any) {
    completePreparation({ id: row.id })
      .then((res) => {
        if (res.data.code === 1) {
          this.$message.success(
            row.deliveryType === 2 ? '制作完成，订单已完成' : '制作完成，已转入待派送'
          )
          this.dialogVisible = false
          this.init(this.orderStatus)
        } else {
