<template>
  <div class="dashboard-container">
    <div class="container">
      <div class="tableBar">
        <label style="margin-right: 10px">甜品名称：</label>
        ...
        <label style="margin-right: 10px; margin-left: 20px">甜品分类：</label>
...
export default class extends Vue {
  ...
  private deleteHandle(type: string, id: any) {
    ...
    this.$confirm('确认删除该甜品, 是否继续?', '确定删除', {
  ...
  private statusHandle(row: any) {
    ...
    if (typeof row === 'string') {
      if (this.checkList.length === 0) {
        this.$message.error('批量操作，请先勾选操作甜品！')
        return false
      }
    ...
    this.$confirm('确认更改该甜品状态?', '提示', {
      ...
      dishStatusByStatus(this.dishState)
        .then(res => {
          if (res && res.data && res.data.code === 1) {
            this.$message.success('甜品状态已经更改成功！')
            this.init()
          } else {
            this.$message.error(res.data.msg)
          }
