...
export default class extends Vue {
  ...
  get rules() {
    return {
      name: [
        {
          ...
          validator: (rule: any, value: string, callback: Function) => {
            if (!value) {
              callback(new Error('请输入甜品名称'))
            } else {
              const reg = /^([A-Za-z0-9\u4e00-\u9fa5]){2,20}$/
              if (!reg.test(value)) {
                callback(new Error('甜品名称输入不符，请输入2-20个字符'))
              } else {
                callback()
              }
            }
      ...
      categoryId: [
        { required: true, message: '请选择甜品分类', trigger: 'change' }
      ],
      image: {
        required: true,
        message: '甜品图片不能为空'
  ...
  private submitForm(formName: any, st: any) {
    ;(this.$refs[formName] as any).validate((valid: any) => {
      ...
      if (valid) {
        if (!this.ruleForm.image) return this.$message.error('甜品图片不能为空')
